...
class DecoratedComponent implements Component {
	...
	set(inner: Component | undefined, style: ToolStyle, policy: ResolvedSlotConfig, brief: string, hint: string): void {
		this.inner = inner;
		this.style = style;
		this.policy = policy;
		this.brief = brief;
...
function rowOf(context: RenderContext, theme: Theme, tool: ResolvedSlotConfig["tool"]): ToolRowComponent {
	const state = stateOf(context);
	if (!state.row) state.row = new ToolRowComponent(theme, tool !== "edit");
	return state.row;
}
		...
		: policy.style === "brief"
			...
			: policy.style === "full"
				...
				: (() => {
					...
					decorator.set(inner, policy.style, policy, brief, _hint);
